...
1. Create or reuse a crates.io API token with publish access for `use-optics`.
...
After the first manual crates.io release for `use-optics` exists, the repository
...
- Purpose: confirms `use-optics` already exists on crates.io, then runs
  `release-plz release`
...
```sh
cargo publish -p use-optics --dry-run
```
